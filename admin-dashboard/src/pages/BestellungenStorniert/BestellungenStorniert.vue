          ...
          <v-card class="mb-1">
            ...
            <template v-slot:item="props">
                <tr>
                  ...
                  <td class="text-xs-center">{{ props.item.Zahlungsdatum | moment("LL") }}</td>
                  <td class="text-xs-center">{{ props.item.Marketplace }}</td>
                  <td class="text-xs-center">{{ props.item.Gesamt }}</td>
                  <td class="text-xs-center">{{ props.item.Rechnungsnummer }}</td>
                  <td
                   class="text-xs-right"
...
export default {
  ...
  methods: {
   ...
   created() {
      ...
      .then((res) => {
         this.items = res.data.filter(d => d.OrderStatus === 'Storniert')
      });
    }
}
...
<style src="./BestellungenStorniert.scss" lang="scss"></style>
